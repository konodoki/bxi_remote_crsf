...
#define JS_VELR_AXIS \
    6 // index of joystick data, used for rotation contro, vel of rotation
...
#define JS_START_BT \
    14 // 启动程序                     (button for start programe)
#define JS_LB_BT 6 // LB功能                       (define LB button)
#define JS_RB_BT 7 // RB功能                       (define RB button)

#define JS_SWITCH_X \
    3 // 暂停或继续跳舞                 (stop or continue danceing)
      // 组合LB：跳舞                   (conbine with LB for change to dance)
      // 组合RB：正常模式，走路站立跑步    (conbine with RB for change to
      // normal: walk stand and run)
#define JS_SWITCH_A \
    0 // 组合LB：HOST起身               (conbine with LB for change to dance)
...
#define JS_SWITCH_Y \
    4 // 组合LB：amp行走
      // 组合RB：初始位置模式            (conbine with LB for change to zero
      // position)
...
public:
    ...
    COMPublisher(const char *_js_dev)
        : Node("COM_publisher")
    {
...
private:
    ...
    bool launch_lock = false; // 防止多次启动程序，True时不允许启动程序。
                              // (launch programe lock)

    bool LB_press = false; // 长按改变状态，弹起恢复                   (pressed
                           // for change state, release for recover)
    ...
    void timer_callback()
    {
        ...
        { // initialize a ROS2 message
            ...
            velxy[0] = (js_axis[JS_VELX_AXIS] * JS_VELX_AXIS_DIR) /
                       (double)AXIS_VALUE_MAX;
            velxy[1] = (js_axis[JS_VELY_AXIS] * JS_VELY_AXIS_DIR) /
                       (double)AXIS_VALUE_MAX;
            velr = (js_axis[JS_VELR_AXIS] * JS_VELR_AXIS_DIR) /
                   (double)AXIS_VALUE_MAX;

            velxy[0] = fabs(velxy[0]) >
                               AXIS_DEAD_ZONE / (double)AXIS_VALUE_MAX ?
                           velxy[0] :
                           0;
            velxy[1] = fabs(velxy[1]) >
                               AXIS_DEAD_ZONE / (double)AXIS_VALUE_MAX ?
                           velxy[1] :
                           0;
            velr = fabs(velr) > AXIS_DEAD_ZONE / (double)AXIS_VALUE_MAX ? velr :
                                                                          0;
